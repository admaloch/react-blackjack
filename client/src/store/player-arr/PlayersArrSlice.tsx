...
import {
  PlayerAndDealerProps,
  PlayerInterface,
  RoundResultsProps,
} from "../../models/PlayerProps";
...
import updatePlayersHelper from "./updatePlayersHelper";
...
const playerArrSlice = createSlice({
  ...
  reducers: {
    ...
    updateWinOrLose: (state, action: PayloadAction<PlayerAndDealerProps>) => {
      ...
      const isRoundWonChanged =
        winOrLoseStr === "Won"
          ? currPlayer.roundsWon + 1
          : currPlayer.roundsWon;
      state[index] = {
        ...state[index],
    ...
    updateAllPlayers: (state) => {
      return updatePlayersHelper(state);
    },

    updatePlayersFromPrevGame: (_state, action) => {
      const playersArr: PlayerInterface[] = action.payload;
      return updatePlayersHelper(playersArr);
    },
    ...
    updateHandResults: (state, action: PayloadAction<PlayerInterface>) => {
      ...
      const playerHasBJ =
        cardSum === 21 && cardUrlVals.length === 2 ? true : false;
...
export const {
  ...
  reverseCurrSplitHand,
  updatePlayerInsurance,
  updateHandResults,
  makeDoubleDownFalse,
  resetAfterInsuranceWin,
  updateWinOrLose,
  updateDoubleDownHand,
  splitPlayerHand,
  updateTokens,
  clickTokenUpdate,
