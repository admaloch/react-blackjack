...
export default function FinalPlayerStats({ player }: FinalPlayerStatsProps) {
  ...
  const finalBankStr =
    bank > 1000 ? (
      <span>
        Amount won: <span className="win-color">${bank - 1000}</span>{" "}
      </span>
    ) : (
      <span>
        Amount lost: <span className="lose-color">${1000 - bank}</span>{" "}
      </span>
    );
  return (
    <li>
      <h5 className={winOrLoseClass}>
        {name} {winOrLoseStr}{" "}
      </h5>
      <p>
        {roundsWonStr} - {currBankStr} - {finalBankStr}
      </p>
    </li>
  );
}
