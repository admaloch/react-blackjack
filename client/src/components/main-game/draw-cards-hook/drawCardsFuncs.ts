...
// various helper functions for draw cards hooks
// Helper function to draw and update a hand
const genCardLocationIndexes = (
  deck: CardObjInterface[],
): DeckLocationInterface => {
...
const changeAceVal = (handInput: Hand) => {
  ...
  if (handWithAlteredAceVals.cardSum > 21) {
    while (handWithAlteredAceVals.cardSum > 21) {
      ...
      handWithAlteredAceVals.cardSum =
        handWithAlteredAceVals.cardNumVals.reduce((a, b) => a + b);
    }
  }
  return handWithAlteredAceVals;
};
const updateDeckFromState = (
  deck: CardObjInterface[],
  cardIndex: number,
  suitIndex: number,
) => {
  return deck.map((card, i) =>
    i === cardIndex
      ? {
          ...card,
          suits: card.suits.map((suit, i) =>
            i === suitIndex ? suit - 1 : suit,
          ),
        }
      : card,
  );
