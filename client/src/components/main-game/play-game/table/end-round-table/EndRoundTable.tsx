...
export default function EndRoundTable() {
  ...
  const isPlayerSplitButNotInsured = playersArr.some(
    (x) => x.splitHand.cards.length > 0 && !x.wonInsuranceRound,
  );
    ...
    <section className="player-results-table">
      ...
      <div className="player-hand-results">
        {playersArr.map((player) => (
          <PlayerHandResults key={player.name} player={player} />
        ))}
      </div>
    </section>
  );
