    ...
    return (
        <div className="player-header">
            {isSplitHand && showSplitHand &&
                <div className={`main-hand-icon ${isRoundActive && 'disabled'}`} onClick={changeToMainHand}>
                    <ShowMainHandIconWithPopper />
                </div>
            }
            <h3>{player.name}</h3>
            {isSplitHand && !showSplitHand &&
                <div className={`split-hand-icon ${isRoundActive && 'disabled'}`} onClick={changeToSplitHand}>
